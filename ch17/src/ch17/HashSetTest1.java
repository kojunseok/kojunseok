...
public class HashSetTest1 {
	public static void main(String[] args) {
		...
		System.out.println("odd에 5추가 성공? " + setChanged); // false -> 중복을 허용하지 않는다.
		
		setChanged = even.add(12);
		System.out.println("even에 12추가 성공? " + setChanged); // false -> 중복을 허용하지 않는다.
		...
		while(odd_it.hasNext()) {
			sum1 = sum1 + odd_it.next();
		}
		System.out.println("odd 집합의 합 : " + sum1);
		...
		while(even_it.hasNext()) {
			sum2 = sum2 + even_it.next();
		}
		System.out.println("even 집합의 합 : " + sum2);
	}
}